...
from my_project.common.basemodel.triton_kernel.destindex_copy_kv import destindex_copy_kv
...
class TransformerLayerInferTpl(TransformerLayerInfer):
    ...
    def __init__(self, layer_num, tp_rank, world_size, network_config, mode, flash_attention=True):
        super().__init__(layer_num, tp_rank, world_size, network_config, mode, flash_attention)
    ...
    def _context_flashattention_kernel(self, q, kv, infer_state: InferStateInfo, layer_weight, out=None) -> torch.Tensor:
        raise Exception("need to impl")
    ...
    def _context_flashattention(self, input_embding, infer_state: InferStateInfo, layer_weight):
        input1 = self._att_norm(input_embding, infer_state, layer_weight)
        cache_kv = self._pre_cache_kv(infer_state, layer_weight)
        q, cache_kv = self._get_qkv(input1, cache_kv, infer_state, layer_weight)
        input1 = None
        self._post_cache_kv(cache_kv, infer_state, layer_weight)
        o = self._context_flashattention_kernel(q, cache_kv, infer_state, layer_weight)
        q = None
        o = self._get_o(o, infer_state, layer_weight)
        if self.world_size_ > 1:
            dist.all_reduce(o, op=dist.ReduceOp.SUM, async_op=False)

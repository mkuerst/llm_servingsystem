...
from my_project.common.req_manager import ReqManager
...
from my_project.common.basemodel.triton_kernel.copy_kv_index_to_req import copy_kv_index_to_req
from my_project.common.basemodel.triton_kernel.splitfuse_copy_kv_index_to_req import splitfuse_copy_kv_index_to_req
...
class TpPartBaseModel():
    ...
    def __init__(self, kvargs):
        self.tp_rank_ = kvargs.get("tp_rank", None)
        self.world_size_ = kvargs.get("world_size", 1)
        self.weight_dir_ = kvargs.get("weight_dir", None)
        # TODO determine max_total_token_num
        self.max_total_token_num = kvargs.get("max_total_token_num", 18400)
        ...
        self.config = kvargs.get("config", None)
        self.flash_attention = kvargs.get("flash_attention", True)
        self.max_new_tokens = kvargs.get("max_new_tokens", 1024)
    ...
    def _init_req_manager(self):
        self.req_manager = ReqManager(self.max_req_num, self.max_seq_length, self.max_new_tokens, self.mem_manager)
        return
    ...
    def _init_infer_layer(self):
        ...
        self.layers_infer = [
            self.transformer_layer_infer_class(
                i, tp_rank=self.tp_rank_, world_size=self.world_size_, network_config=self.config, mode=self.mode, flash_attention=self.flash_attention
            )
            for i in range(self.config["n_layer"])
        ]
        return
    ...
    def _prefill(
        ...
        return predict_logics, elapsed_time
